<!-- HEADER -->
<div align="center">

<br>

<!-- TITLE -->
<h1>Awesome OpenCLI</h1>

<!-- BADGES -->
<p>
<a href="https://github.com/sindresorhus/awesome"><img src="https://cdn.rawgit.com/sindresorhus/awesome/d7305f38d29fed78fa85652e3a63e154dd8e8829/media/badge.svg" height="28" alt="Awesome" /></a>
&nbsp;&nbsp;
<a href="https://github.com/jackwener/opencli"><img src="https://img.shields.io/github/stars/jackwener/opencli?style=social" height="28" alt="OpenCLI Stars" /></a>
</p>

<br>

<!-- DESCRIPTION -->
<h3>A curated list of plugins, skills, tools, and resources for <a href="https://github.com/jackwener/opencli">OpenCLI</a>.</h3>
<h3>Make any website, Electron App, or Local Tool your CLI.</h3>

<br>

[**OFFICIAL**](#official) • [**WEBSITE**](#website) • [**PLUGINS**](#plugins) • [**SKILLS**](#skills) • [**APPS**](#apps)

<br>
<hr>

</div>

<!-- CONTENT -->

<div id="official"></div>

<h3>⭐️ Official Repository</h3>

| Project | Stars | Description |
| :--- | :--- | :--- |
| **[opencli](https://github.com/jackwener/opencli)** | ![Stars](https://badgen.net/github/stars/jackwener/opencli) | The official OpenCLI - Make any website, Electron App, or Local Tool your CLI. |

<br>

<div id="website"></div>

<h3>🌐 Website</h3>

| Project | Description |
| :--- | :--- |
| **[opencli-website](https://github.com/jackwener/opencli-website)** | Official website for OpenCLI. |

<br>

<div id="plugins"></div>

<h3>🧩 Plugins</h3>

- [`opencli-plugin-upwork`](https://github.com/Alpha-Innovation-Labs/opencli-plugin-upwork) - Search and read Upwork jobs with OpenCLI.
- [`opencli-plugin-12306`](https://github.com/Hwenyi/opencli-plugin-12306) - OpenCLI plugin for 12306.
- [`opencli-plugin-xhs`](https://github.com/CHENXCHEN/opencli-plugin-xhs) - Xiaohongshu plugin.
- [`opencli-plugin-hot-digest`](https://github.com/ByteYue/opencli-plugin-hot-digest) - Cross-platform trending aggregator.
- [`opencli-plugin-juejin`](https://github.com/Astro-Han/opencli-plugin-juejin) - Juejin plugin.
- [`opencli-plugin-stockanalysis`](https://github.com/haskaomni/opencli-plugin-stockanalysis) - StockAnalysis plugin.
- [`opencli-plugin-vk`](https://github.com/flobo3/opencli-plugin-vk) - VK plugin.
- [`opencli-plugin-gemini-web`](https://github.com/AstaTus/opencli-plugin-gemini-web) - Gemini web plugin.
- [`opencli-plugin-audit-log`](https://github.com/ByteYue/opencli-plugin-audit-log) - Command audit logging.
- [`opencli-plugin-usage-analytics`](https://github.com/ByteYue/opencli-plugin-usage-analytics) - Usage analytics plugin.
- [`opencli-plugin-slack-notify`](https://github.com/ByteYue/opencli-plugin-slack-notify) - Slack and Telegram notification plugin.
- [`opencli-plugin-github-trending`](https://github.com/ByteYue/opencli-plugin-github-trending) - GitHub trending from the terminal.
- [`opencli-plugin-data-diff`](https://github.com/ByteYue/opencli-plugin-data-diff) - Track command output changes between runs.
- [`opencli-plugin-bohao-work`](https://github.com/gnosis23/opencli-plugin-bohao-work) - Bohao.work blog posts plugin.
- [`boss-zhipin-cli`](https://github.com/tianhhe/boss-zhipin-cli) - BOSS Zhipin job site CLI.
- [`TW-Judgment-cli`](https://github.com/show940125/TW-Judgment-cli) - Taiwan judicial judgments extension.

<br>

<div id="skills"></div>

<h3>🛠️ Skills</h3>

- [`opencli-skill`](https://github.com/joeseesun/opencli-skill) - Related OpenCLI ecosystem project.
- [`opencli-skill`](https://github.com/ouerzc/opencli-skill) - Cross-platform OpenCLI skill kit for AI agents.
- [`opencli-skill`](https://github.com/macrochen/opencli-skill) - Local wrapper with searchable HTML guide.
- [`wechat-opencli-fetcher-skill`](https://github.com/purebluen73/wechat-opencli-fetcher-skill) - WeChat article scraping skill.

<br>

<div id="apps"></div>

<h3>🚀 Apps</h3>

- [`opencli-admin`](https://github.com/xjh1994/opencli-admin) - Visual content collection and AI tagging system for sentiment monitoring.
- [`deepfind-cli`](https://github.com/liuslevis/deepfind-cli) - Multi Agent Research using opencli.
- [`opencli-desktop-bridges`](https://github.com/sk1935/opencli-desktop-bridges) - Control desktop apps via OpenCLI (Grok, Codex, Antigravity).
- [`gemini-image-cli`](https://github.com/b619-y/gemini-image-cli) - Gemini image generation based on OpenCLI browser adapter.
- [`nightly-mvp-2026-03-19-opencli`](https://github.com/sys-fairy-eve/nightly-mvp-2026-03-19-opencli) - Web-based terminal inspired by OpenCLI.
- [`jelp`](https://github.com/scottvr/jelp) - CLI introspection via OpenCLI schema.
- [`twitter-to-obsidian`](https://github.com/MaoYo42/twitter-to-obsidian) - Twitter bookmarks to Obsidian using OpenCLI.

<br>

## Contributing

Please see [`contributing.md`](contributing.md).
